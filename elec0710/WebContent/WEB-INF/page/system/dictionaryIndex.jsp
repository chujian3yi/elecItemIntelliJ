	...
	<HEAD>
		...
		function changetype(){
		  ...
		  if(document.Form1.keyword.value=="jerrynew"){
		     ...
		     /*
		     	*Pub.submitActionWithForm：封装的js(ajax)
		     	*参数一：表单Form2的名称
		     	*参数二：url路径
		     	*参数三：表单Form1的名称
		     	*用法：
		     		*1:在dictionaryIndex.jsp中存在2个表单，分别叫Form1和Form2.
		     		*2:要求传递表单Form1中的元素的名称和值作为参数，传递给服务器，在服务器端进行处理，将处理后的结果，跳转到dictionaryEdit.jsp中
		     		*3:将dictionaryEdit.jsp中的内容全部嵌套（innerHTML）到dictionaryIndex.jsp的Form2中
		       */
		     Pub.submitActionWithForm('Form2','elecSystemDDLAction_edit.do','Form1');
		    
		  }else{
		    //此时，选择了类型列表，表示：在已有的数据类型基础撒很难过进行修该和编辑
		    var textStr="";
...
function delTableRow(rowNum){ 
				...
				<tr>
					...
					<td class="ta_01" align="left"  width="30%" >
						...
						<s:if test="#request.systemList!=null && #request.systemList.size()>0">
							<s:select list="#request.systemList" name="keyword" id="keyword"
 ...
 <s:form name="Form2" id="Form2"  namespace="/system" method="post" cssStyle="margin:0px;">
    <table cellSpacing="1" cellPadding="0" width="90%" align="center" bgColor="#f5fafe" border="0" >
    <tr>
     <td >
